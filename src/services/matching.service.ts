...
import { RoommateProfile, Match, User } from '../models';
...
export class MatchingService {
  ...
  static calculateCompatibilityScore(
    ...
    const budgetScore = this.calculateBudgetScore(
      profile1.matchingPreferences.budget,
      profile2.matchingPreferences.budget
    );
    ...
    const locationScore = this.calculateLocationScore(
      ...
      profile1.matchingPreferences.maxDistance
    );
    ...
    const interestsScore = this.calculateInterestsScore(
      profile1.interests,
      profile2.interests
    );
    ...
    const moveInScore = this.calculateMoveInDateScore(
      profile1.matchingPreferences.moveInDate,
      profile2.matchingPreferences.moveInDate
    );
    totalScore += moveInScore;
    breakdown.moveInDate = moveInScore;

    // 6. Lease duration compatibility (10 points)
    const leaseDurationScore = this.calculateLeaseDurationScore(
      profile1.matchingPreferences.leaseDuration,
      profile2.matchingPreferences.leaseDuration
    );
  ...
  private static calculateBudgetScore(budget1: any, budget2: any): number {
  ...
  private static calculateLifestyleScore(lifestyle1: any, lifestyle2: any): number {
    let score = 0;
    ...
    if (lifestyle1.sleepSchedule === lifestyle2.sleepSchedule) {
      ...
      (lifestyle1.sleepSchedule === 'moderate' || lifestyle2.sleepSchedule === 'moderate')
    ) {
      score += 3;
    }
    ...
    if (lifestyle1.socialLevel === lifestyle2.socialLevel) {
      ...
      lifestyle1.socialLevel === 'ambivert' || lifestyle2.socialLevel === 'ambivert'
    ) {
      score += 2.5;
    }
    ...
    if (lifestyle1.drinking === lifestyle2.drinking) {
      ...
      (lifestyle1.drinking === 'occasionally' || lifestyle2.drinking === 'occasionally')
    ) {
      score += 2;
    }
  ...
  private static calculateMoveInDateScore(date1: Date, date2: Date): number {
    ...
    if (diffDays <= 60) return 2; // Within 2 months
    return 0;
  }

  // Lease duration compatibility
  private static calculateLeaseDurationScore(duration1: number, duration2: number): number {
    const diffMonths = Math.abs(duration1 - duration2);

    if (diffMonths === 0) return 10;
    if (diffMonths <= 3) return 7;
    if (diffMonths <= 6) return 4;
    return 0;
  }
  ...
  static async findMatches(
    ...
    try {
      ...
      const potentialMatches = await RoommateProfile.find(query)
  ...
  static async createMatch(
    ...
    try {
      // Get profiles
      const profile1 = await RoommateProfile.findOne({ user: userId1 });
      const profile2 = await RoommateProfile.findOne({ user: userId2 });
