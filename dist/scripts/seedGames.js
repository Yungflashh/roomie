...
async function seedGames() {
    try {
        ...
        await models_1.Game.deleteMany({});
        logger_1.logger.info('Cleared existing games');
        // Insert sample games
        const games = await models_1.Game.insertMany(sampleGames);
        logger_1.logger.info(`Successfully seeded ${games.length} games`);
        await mongoose_1.default.connection.close();
        logger_1.logger.info('Database connection closed');
        process.exit(0);
    }
    catch (error) {
        logger_1.logger.error(`Error seeding games: ${error}`);
        process.exit(1);
    }
}
seedGames();
