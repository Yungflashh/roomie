...
class GameSessionController {
    ...
    static async updateUserStats(session) {
        const game = await models_1.Game.findById(session.game);
        if (!game)
            return;
...
_a = GameSessionController;
// Create game session
GameSessionController.createSession = (0, catchAsync_1.default)(async (req, res) => {
    const userId = req.user?.id;
    const { gameId, invitedUsers, scheduledFor } = req.body;
    const game = await models_1.Game.findById(gameId);
    if (!game) {
    ...
    if (invitedUsers && invitedUsers.length > 0) {
        const invitePromises = invitedUsers.map(async (invitedUserId) => {
            ...
            socketService.emitToUser(invitedUserId, 'game-invite', {
                session,
                game,
                host: await models_1.User.findById(userId).select('firstName lastName profilePicture'),
            });
        });
        await Promise.all(invitePromises);
    }
    apiResponse_1.ApiResponse.created(res, { session }, 'Game session created successfully');
});
// Join game session
GameSessionController.joinSession = (0, catchAsync_1.default)(async (req, res) => {
    const userId = req.user?.id;
    const { sessionId } = req.params;
    const session = await models_1.GameSession.findById(sessionId);
    if (!session) {
        throw new ApiError_1.default(404, 'Game session not found');
    }
    if (session.status !== 'waiting') {
        throw new ApiError_1.default(400, 'Cannot join session that has already started or completed');
    }
    const game = await models_1.Game.findById(session.game);
    if (!game) {
        throw new ApiError_1.default(404, 'Game not found');
    ...
    if (alreadyJoined) {
        throw new ApiError_1.default(400, 'You have already joined this session');
    }
    // Check max players
    if (session.participants.length >= game.maxPlayers) {
        throw new ApiError_1.default(400, 'This game session is full');
    }
    // Add participant
    ...
    io.to(`game:${sessionId}`).emit('player-joined', {
        sessionId,
        participant: await models_1.User.findById(userId).select('firstName lastName profilePicture'),
    });
    apiResponse_1.ApiResponse.success(res, { session }, 'Joined game session successfully');
});
// Leave game session
GameSessionController.leaveSession = (0, catchAsync_1.default)(async (req, res) => {
    const userId = req.user?.id;
    const { sessionId } = req.params;
    const session = await models_1.GameSession.findById(sessionId);
    if (!session) {
    ...
    if (session.status !== 'waiting') {
        throw new ApiError_1.default(400, 'Cannot leave session that has already started');
    }
    // Remove participant
    ...
    apiResponse_1.ApiResponse.success(res, null, 'Left game session successfully');
});
// Start game session
GameSessionController.startSession = (0, catchAsync_1.default)(async (req, res) => {
    const userId = req.user?.id;
    const { sessionId } = req.params;
    const session = await models_1.GameSession.findById(sessionId);
    if (!session) {
    ...
    if (session.host.toString() !== userId) {
        throw new ApiError_1.default(403, 'Only the host can start the game');
    }
    if (session.status !== 'waiting') {
        throw new ApiError_1.default(400, 'Game has already started or completed');
    }
    const game = await models_1.Game.findById(session.game);
    if (!game) {
        throw new ApiError_1.default(404, 'Game not found');
    ...
    if (session.participants.length < game.minPlayers) {
        throw new ApiError_1.default(400, `Need at least ${game.minPlayers} players to start this game`);
    }
    // Start session
    ...
    apiResponse_1.ApiResponse.success(res, { session }, 'Game started successfully');
});
// Submit answer
GameSessionController.submitAnswer = (0, catchAsync_1.default)(async (req, res) => {
    const userId = req.user?.id;
    const { sessionId } = req.params;
    ...
    if (session.status !== 'in-progress') {
        throw new ApiError_1.default(400, 'Game is not in progress');
    }
    const game = await models_1.Game.findById(session.game);
    if (!game) {
        throw new ApiError_1.default(404, 'Game not found');
    ...
    if (participantIndex === -1) {
        throw new ApiError_1.default(403, 'You are not a participant in this game');
    }
    // Find question
    const question = game.questions?.find((q) => q.question === questionId);
    if (!question) {
        throw new ApiError_1.default(404, 'Question not found');
    }
    // Check if already answered
    const alreadyAnswered = session.participants[participantIndex].answers.some((a) => a.questionId === questionId);
    if (alreadyAnswered) {
        throw new ApiError_1.default(400, 'You have already answered this question');
    }
    // Calculate score
    let isCorrect = false;
    let pointsEarned = 0;
...
GameSessionController.completeSession = (0, catchAsync_1.default)(async (req, res) => {
    const userId = req.user?.id;
    const { sessionId } = req.params;
    const session = await models_1.GameSession.findById(sessionId);
    if (!session) {
    ...
    if (session.host.toString() !== userId && session.status !== 'in-progress') {
        throw new ApiError_1.default(403, 'Only the host can complete the game');
    }
    if (session.status === 'completed') {
        throw new ApiError_1.default(400, 'Game has already been completed');
    }
    // Calculate rankings
    const sortedParticipants = [...session.participants].sort((a, b) => b.score - a.score);
    sortedParticipants.forEach((participant, index) => {
        const participantIndex = session.participants.findIndex((p) => p.user.toString() === participant.user.toString());
    ...
    await session.save();
    // Update user stats
    await _a.updateUserStats(session);
    await session.populate('game');
    await session.populate('participants.user', 'firstName lastName profilePicture');
    ...
    apiResponse_1.ApiResponse.success(res, { session, results: sortedParticipants }, 'Game completed successfully');
});
// Get user's game sessions
GameSessionController.getMySessions = (0, catchAsync_1.default)(async (req, res) => {
    const userId = req.user?.id;
    ...
    const sessions = await models_1.GameSession.find(query)
        .populate('game')
        ...
        .skip(skip)
        .limit(Number(limit));
    const total = await models_1.GameSession.countDocuments(query);
    apiResponse_1.ApiResponse.paginated(res, sessions, Number(page), Number(limit), total, 'Game sessions retrieved successfully');
});
// Get session by ID
GameSessionController.getSessionById = (0, catchAsync_1.default)(async (req, res) => {
    const userId = req.user?.id;
    const { sessionId } = req.params;
    const session = await models_1.GameSession.findById(sessionId)
        .populate('game')
    ...
    if (!isParticipant && !isInvited && !isHost) {
        throw new ApiError_1.default(403, 'You do not have access to this game session');
    }
    apiResponse_1.ApiResponse.success(res, { session }, 'Game session retrieved successfully');
});
