...
class ElasticsearchService {
    constructor() {
        this.indexName = 'roommate_profiles';
    }
    // Check if Elasticsearch is available
    isAvailable() {
        return elasticsearch_1.elasticsearchClient.isAvailable();
    }
    // Create index with mappings
    async createIndex() {
        if (!elasticsearch_1.esClient)
            return;
        try {
            const exists = await elasticsearch_1.esClient.indices.exists({ index: this.indexName });
            if (!exists) {
            ...
            logger_1.logger.error('Error creating Elasticsearch index:', error);
            throw error;
        }
    }
    // Index a profile
    async indexProfile(profileId) {
        if (!elasticsearch_1.esClient)
            return;
        try {
            const profile = await models_1.RoommateProfile.findById(profileId).populate('user');
            if (!profile) {
                logger_1.logger.warn(`Profile ${profileId} not found for indexing`);
                return;
            }
            ...
            logger_1.logger.info(`Profile ${profileId} indexed successfully`);
        }
        catch (error) {
            logger_1.logger.error(`Error indexing profile ${profileId}:`, error);
        }
    }
    // Bulk index all profiles
    async bulkIndexProfiles() {
        if (!elasticsearch_1.esClient)
            return;
        try {
            const profiles = await models_1.RoommateProfile.find({ isProfileComplete: true }).populate('user');
            const body = profiles.flatMap((profile) => {
                const user = profile.user;
                return [
                    { index: { _index: this.indexName, _id: profile._id.toString() } },
                    {
                        userId: user._id.toString(),
                        profileId: profile._id.toString(),
                        firstName: user.firstName,
            ...
            if (body.length > 0) {
                const { errors } = await elasticsearch_1.esClient.bulk({ refresh: true, body });
                if (errors) {
                    logger_1.logger.error('Bulk indexing had errors');
                }
                else {
                    logger_1.logger.info(`Successfully indexed ${profiles.length} profiles`);
                }
            }
        }
        catch (error) {
            logger_1.logger.error('Error bulk indexing profiles:', error);
            throw error;
        }
    }
    ...
    async deleteProfile(profileId) {
        ...
        try {
            ...
            logger_1.logger.info(`Profile ${profileId} deleted from index`);
        }
        catch (error) {
            if (error.meta?.statusCode !== 404) {
                logger_1.logger.error(`Error deleting profile ${profileId}:`, error);
            }
        }
    }
    // Search profiles
    async searchProfiles(searchQuery) {
        ...
        const { query, filters = {}, location, sort = 'relevance', page = 1, limit = 20, } = searchQuery;
        const must = [];
        const filter = [];
        // Text search
        if (query && query.trim()) {
    ...
    async autocompleteSuggestions(prefix, field = 'firstName') {
        if (!elasticsearch_1.esClient || !prefix || prefix.length < 2) {
            return [];
        }
        try {
            const result = await elasticsearch_1.esClient.search({
            ...
            logger_1.logger.error('Autocomplete error:', error);
            return [];
        }
    }
    // Get aggregations
    async getAggregations() {
        ...
        try {
            ...
            logger_1.logger.error('Aggregations error:', error);
            throw error;
        }
    }
}
