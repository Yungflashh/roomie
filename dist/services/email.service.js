...
class EmailTemplateService {
    ...
    compileTemplate(templateName, data) {
        try {
            // Read base template
            const basePath = path_1.default.join(__dirname, '../templates/emails/base.mjml');
            const baseTemplate = fs_1.default.readFileSync(basePath, 'utf8');
            // Read content template
            const contentPath = path_1.default.join(__dirname, `../templates/emails/${templateName}.mjml`);
            const contentTemplate = fs_1.default.readFileSync(contentPath, 'utf8');
            // Compile content with Handlebars
            const contentCompiled = handlebars_1.default.compile(contentTemplate);
            ...
            const fullCompiled = handlebars_1.default.compile(fullTemplate);
    ...
    async sendMatchEmail(email, firstName, matchData) {
        await this.sendEmail({
            to: email,
    ...
    async sendDailyDigest(email, firstName, digestData) {
        await this.sendEmail({
            to: email,
